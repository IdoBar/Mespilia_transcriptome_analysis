---
title: "Transcriptome analyses of sea urchin larvae"
author: "Tessa Page and Ido Bar"
...
output: 
  ...
  #    highlight: pygments
      ...
      code_folding: hide
#      keep_md: true
bibliography: ["style/RNAseq_references.bib", "style/pseudo_counts.bib", "style/comparative_genomics.bib", "style/trans_assembly_papers.bib", "style/transcriptome_assembly_evaluation.bib"]
...
-   Identify genes that play a role in the development of sea urchin (_Mespilia globulus_) larvae      

### Objectives

1. Assemble and annotate the _M. globulus_ larval transcriptome
...
RNA was extracted from different larval stages of XX sea urchin (_Mespilia globulus_). RNA was extracted using the XXXX method. The RNA was sent for Illumina paired-end sequencing, generating XXXbp reads (specify sequence provider). 
...
Since no reference genome (and transcriptome) exists or _M. globolus_, we assembled a _De-Novo_ transcriptome assembly from the samples.   
*To reduce the computational load, a subset of 25% of the reads from each sample can be used.*   
...
#### Predict open reading frames with TransDecoder2
The gene annotation of the assembled transcriptome was 'sanitized' using `gffread` [@perteaGFFUtilitiesGffRead2020], followed by [TransDecoder2](https://github.com/Markusjsommer/TD2 v1.0.5 to predict open reading frames of proteins (see detailed instructions in the [Documentation](https://github.com/Markusjsommer/TD2/wiki) and the publication by @maoTD2FindingProtein2025).  
...
# setup environment
CONDA_NAME="genomics"
mamba install -n $CONDA_NAME td2 
ASS="Mnova_ref_based_assembly"
...
JOBNAME="mmseq2"
# use container docker://ghcr.io/soedinglab/mmseqs2:master-cuda12
NCORES=8
MEM=32
WALLTIME=10:00:00
echo "
apptainer run docker://ghcr.io/soedinglab/mmseqs2:master-cuda12 -B ~/adna/tools/ncbi_db mmseqs databases NR swissprot \$TMPDIR
apptainer run docker://ghcr.io/soedinglab/mmseqs2:master-cuda12 -B ~/adna/tools/ncbi_db mmseqs easy-search transcripts.TD2_dir/longest_orfs.pep swissprot alnRes.m8 \$TMPDIR -s 7.0
...
JOBNAME="td2-predict"
echo "cat file1.m8 file2.m8 > combined_alnRes.m8
...
The assembled transcriptome was annotated using BLASTn v2.16.0 [@camachoBLASTArchitectureApplications2009] against the non-redundant nucleotide database of the NCBI (`nt`) to achieve more accurate species-specific annotations. 
...
# send results back to Sharepoint 
...
This document was last updated at `r Sys.time()` using R Markdown (built with `r R.version.string`). The source code for this webpage can be found at <https://github.com/IdoBar/Mespilia_transcriptome_analysis> (or via the GitHub logo at the top right corner of this page).  
